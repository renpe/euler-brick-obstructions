"""
Factorise f1_prim = x_prim² + y_prim² + z_prim² (primitive brick
space-diagonal squared) for every Master-Hit. Uses the algebraic
identity
    f1_prim = L_prim · R_prim
with L_prim = (C/g_-)² + (D/g_-)², R_prim = (A/g_+)² + (B/g_+)²
(primitive sums of two squares, with half the digit count).
...
Writes to pub.prim_brick_factors and updates num_blockers.
...
def process_hit(row):
    """Factorises f1_prim = x_prim² + y_prim² + z_prim² via the algebraic
    identity f1_prim = L_prim · R_prim, with
        L_prim = ((am-bn)/g_-)² + ((an-bm)/g_-)²    (g_- = gcd(C,D))
        R_prim = ((am+bn)/g_+)² + ((an+bm)/g_+)²    (g_+ = gcd(A,B))
    Each factor is run separately through the trial-div -> sympy -> YAFU
    pipeline; the factor dicts are then merged multiplicatively. Status
    'full' only if both sub-factorisations are 'full'.
    """
    ...
    try:
        ...
        # algebraic split via g_+, g_-
        from math import gcd
        A = a*m + b*n; B = a*n + b*m
        C = a*m - b*n; D = a*n - b*m
        gp = gcd(A, B)
        gm = gcd(abs(C), abs(D))
        # L_prim and R_prim as primitive sums of two squares
        Cp = C // gm; Dp = D // gm
        Ap = A // gp; Bp = B // gp
        L_prim = Cp*Cp + Dp*Dp
        R_prim = Ap*Ap + Bp*Bp
        # Sanity check: L_prim · R_prim == f1_prim (algebraic identity)
...
def main():
    ...
    cur = conn.cursor()
    # We factorise f1_prim = x_prim²+y_prim²+z_prim² for every Master-Hit
    # including non-primitive ones (g_scale > 1), since the standard
    # parametrisation (a,b,m,n) does not directly produce every primitive
    # Euler brick — Saunderson bricks, for instance, appear only as
    # scaled versions, and their f1 = g_scale² · f1_prim still carries
    # the information about the primitive root brick. f1_prim is the
    # essential quantity for Conjecture B (g_scale² is a square and
    # contributes nothing to the blocker structure).
    cur.execute("""
